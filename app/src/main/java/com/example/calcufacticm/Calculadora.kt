...
class Calculadora : AppCompatActivity() {
    val res: TextView? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_calculadora)
        val res: TextView = findViewById((R.id.tvFact))
    }

    fun calcular (view: View){
        var boton = view as Button
        var textoBoton = boton.text.toString()
        var concatenar = res?.text.toString()+textoBoton
        var mostrar = quitarCero(concatenar)
        if (textoBoton == "="){
            var resultado = 0.0
            try {
                resultado = eval(res?.text.toString())
                res?.text = resultado.toString()
            }catch (e:Exception){
                res?.text = "Digita bien oee!!"
            }
    ...
    fun quitarCero (str:String):String{
        var i = 0
        while (i < str.length && str[i] == '0') i++
        // Buffer para reemplazar los ceros desde el principio hasta donde encuentre ceros por null
